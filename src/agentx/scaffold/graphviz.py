...
#: The manifest schema version this build of agentx-kit knows how to read.
#: Manifests written before this field existed are treated as version 1 (the
#: original, still-current shape) — only a *future* version (written by a
#: newer agentx-kit than the one reading it) is rejected, so upgrading the
#: scaffolder never silently misinterprets a manifest whose shape changed.
MANIFEST_VERSION = 1
...
def load_manifest(project: Path | None) -> tuple[Path, dict]:
    ...
    Raises FileNotFoundError if no manifest is found, or ValueError if the
    manifest declares a schema version newer than this install understands.
    """
    ...
    for parent in [base, *base.parents]:
        ...
        if mf.exists():
            ...
            if version > MANIFEST_VERSION:
                raise ValueError(
                    f"{mf} was generated by a newer agentx-kit (manifest_version={version}); "
                    f"this install only understands up to version {MANIFEST_VERSION}. "
                    "Upgrade with: pip install -U agentx-kit"
                )
