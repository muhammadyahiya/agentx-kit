...
def _snippet(file: str | None, lineno: int | None, *, context: int = 4) -> str:
    """A few source lines around ``lineno``, read once at generation time so
    the viewer needs no live file access."""
...
def _payload(flow: Flow) -> dict:
    ...
    for name, node in flow.nodes.items():
        ...
        nodes.append({
            ...
            "snippet": _snippet(node.file, node.lineno),
        })
    edges = [
...
def render_html(flow: Flow) -> str:
    """Render ``flow`` as one complete, self-contained HTML document."""
    payload_json = json.dumps(_payload(flow)).replace("</", "<\\/")
    vendor_scripts = "\n".join(
...
(function () {
  ...
  function buildElementsForLevel(level) {
    ...
    for (const id of visibleIds) {
      const n = byId[id];
      const d = { id: n.id, label: n.label, kind: n.kind, calls: n.calls };
      let p = n.parent;
  ...
  function showPanel(n) {
    ...
    panel.innerHTML = `<h2>${d.id}</h2><div class="meta">${meta}</div>` +
      (d.snippet ? `<pre>${d.snippet.replace(/[&<>]/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;'}[c]))}</pre>` : '') +
      `<p class="hint">Click a second node to highlight the call path between them.</p>`;
  }
